...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Three cities
        ([("state", "city1", 0.0, 0.0), ("state", "city2", 1.0, 1.0),
...
@pytest.mark.parametrize(
    "road_map,index,expected_road_map,expected_distance",
    [
        # No roadmap
        ([], 0, [], 0.0),
        # Single city
        ([("state", "city", 0.0, 0.0)], 0, [("state", "city", 0.0, 0.0)], 0.0),
        # Two cities
        ([("state", "city1", 0.0, 0.0), ("state", "city2", 1.0, 1.0)], 0, [
            ("state", "city2", 1.0, 1.0), ("state", "city1", 0.0, 0.0)
        ], 195.28443017165606),
    ])
def test_swap_adjacent_cities(road_map, index, expected_road_map,
                              expected_distance):
    original_road_map = copy.copy(road_map)
...
@pytest.mark.parametrize(
    "road_map,index1,index2,expected_road_map,expected_distance",
    [
        # No roadmap
        ([], 0, 1, [], 0.0),
        # Single city
        ([("state", "city1", 0.0, 0.0)], 0, 1, [("state", "city1", 0.0, 0.0)],
         0.0),
        # Two cities
        ([("state", "city1", 0.0, 0.0), ("state", "city2", 1.0, 1.0)], 1, 0, [
            ("state", "city2", 1.0, 1.0), ("state", "city1", 0.0, 0.0)
        ], 195.28443017165606),
    ])
...
@pytest.mark.parametrize(
    "road_map,expected_road_map",
    [
        # No roadmap
        ([], []),
        # Single city
        ([("state", "city1", 0.0, 0.0)], [("state", "city1", 0.0, 0.0)]),
        # Two cities
        ([("state", "city1", 0.0, 0.0),
          ("state", "city2", 1.0, 1.0)], [("state", "city1", 0.0, 0.0),
                                          ("state", "city2", 1.0, 1.0)]),
